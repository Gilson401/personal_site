...
function NavDoApp(props) {
  ...
  /**Monta Dropdown.Item*/
  const montarLinksDrop = () =>
    mainLinks.map((itemMainlinks, index) => (
      <Dropdown.Item
        key={index}
        eventKey={index}
        onClick={() => router.push(itemMainlinks.push)}
      >
        {" "}
        {itemMainlinks.label}{" "}
      </Dropdown.Item>
  ...
  function CollapseNavbarBootstrap() {
    return (
      <STNavbar collapseOnSelect expand="lg" variant="light">
        <Container>
          <Navbar.Brand>
            <img
              className=" d-block w-100 responsive hidesmall"
